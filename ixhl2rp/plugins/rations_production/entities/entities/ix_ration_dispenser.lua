...
ENT.Displays = {
    [1] = {"ОЖИДАНИЕ", color_white, true},
    [2] = {"ПРОВЕРКА", Color(255, 200, 0)},
    [3] = {"ВЫДАЧА", Color(0, 255, 0)},
    [4] = {"ВРЕМ. ЛИМИТ", Color(255, 0, 0)},
    [5] = {"ОЖИДАЙТЕ", Color(255, 200, 0)},
    [6] = {"НЕАКТИВЕН", Color(255, 0, 0), true},
    [7] = {"ОЖИДАНИЕ", Color(255, 0, 0)},
    [8] = {"ПОДГОТОВКА", Color(0, 255, 0)},
    [9] = {"ПР.СКЛАД", Color(255, 200, 0)},
    [10] = {"ПУСТО", Color(200, 0, 0)},
}
...
if (SERVER) then
    ...
    function ENT:SpawnRation(callback, releaseDelay)
        ...
        local itemTable = ix.item.Get("gelsac_s")
        ...
        timer.Simple(releaseDelay, function()
            ix.item.Spawn("gelsac_s", self.dummy:GetPos(), function(item, entity)
                self.dummy:SetNoDraw(true)
